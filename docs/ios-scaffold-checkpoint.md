...
- Producer Search sets expectations that release-level producer-credit checks can take a moment, uses `Open details` result actions, displays counts as results found so far, and suppresses cramped leading numeric title prefixes in result cards.
...
- Manual iOS UI smoke testing reported by the user after `4b928f1` confirmed Album Search no-results handling, Results sort preservation, selected-edition sanity, Release Credits terminology, Producer Search candidate/results/Load More/open-detail flows, Producer Search clear controls, weak-query no-candidate states, and offline Album Search error handling; this is manual validation, not an automated verifier test.
- Signed TestFlight iPhone smoke testing after the `ExpoAsset` startup fix confirmed the app launches on iPhone 11 with no black screen, renders Search / Results / Album Detail, loads R.E.M. `Murmur` Release Credits, Tracklist credits, artwork gallery, and full-screen artwork viewer; very large Dynamic Type / accessibility text sizing remains a non-blocking layout follow-up.
- Signed TestFlight build 3 smoke testing confirmed the app works on iPhone 11 across Album Search, Results, Album Detail, Release Credits, Tracklist credits, artwork gallery/viewer, external Wikipedia links, and Producer Search. Build 3 testing found stale album-search header context while viewing Producer Search results; follow-up fix `1b8833b` was verified in Simulator/dev and will be included in the next TestFlight build.
